...
/* La fotografía se vuelve arena: las partículas nacen sobre sus píxeles,
   ...
   Los números vienen calibrados en Lab-FX (`Partículas.labfx.json`, 2026-07-29)
   y viajan como uniforms en vez de estar cosidos al shader: así el ajuste fino
   se hace desde `AJUSTES` sin tocar GLSL, y lo que se lee aquí es exactamente
   lo que Sergio vio en el visor. */
...
void main() {
  ...
  float angle = simplexNoise(vec3(position * u_noise_scale, u_time * 20.0 + life * .05)) * 6.2831;
  vec2 noiseForce = vec2(cos(angle), sin(angle)) * u_noise;
  vec2 toMouse = position - u_mouse;
  float proximity = u_mouse_radius2 / (dot(toMouse, toMouse) + u_mouse_radius2);
  vec2 mouseForce = u_mouse_velocity * proximity * u_mouse_force;

  velocity = velocity * u_friction + noiseForce + mouseForce;
  position += velocity;
...
export type Ajustes = {
  particulas: number
  /** Fotogramas que vive un grano. El reparto va del 25% de este número al 100%. */
  vida: number
  tamano: number
  opacidad: number
  ruido: number
  ...
  friccion: number
  raton: number
  /** Radio de influencia del puntero en píxeles; el shader usa su cuadrado. */
  radioRaton: number
  velocidad: number
  /** Opacidad de la fotografía que queda debajo de la arena. */
  fondo: number
}
...
export class DesertParticles {
  ...
  private uSize = new Uniform({ name: 'u_size', value: 4, kind: 'float' })
  private uOpacity: Uniform
  private uNoise: Uniform
  private uNoiseScale: Uniform
  ...
  constructor(private options: Options) {
    ...
    this.uNoiseScale = new Uniform({ name: 'u_noise_scale', value: a.escalaRuido, kind: 'float' })
    this.uFriction = new Uniform({ name: 'u_friction', value: a.friccion, kind: 'float' })
    this.uMouseForce = new Uniform({ name: 'u_mouse_force', value: a.raton, kind: 'float' })
    this.uMouseRadius2 = new Uniform({ name: 'u_mouse_radius2', value: a.radioRaton ** 2, kind: 'float' })
  ...
  private build() {
    const { container, image } = this.options
    const a = this.ajustes
    ...
    /* El tope sigue atado al área para que un lienzo pequeño no reserve
       memoria de más, pero con .34 el número pedido manda hasta en pantallas
       muy pequeñas. */
    ...
    const vidaMin = a.vida * .25

    for (let i = 0; i < total; i++) {
      const x = Math.random() * pw
      const y = Math.random() * ph
      const life = vidaMin + Math.random() * (a.vida - vidaMin)
      let u = x / pw
      let v = y / ph
    ...
    const program = new Program(this.gl, {
      ...
      uniforms: {
        u_time: this.uTime, u_resolution: this.uResolution, u_mouse: this.uMouse,
  ...
  private render(time: number) {
    ...
    this.uTime.value = (this.uTime.value as number) + delta * .00005 * this.ajustes.velocidad
